...
bot.on("message",async(msg)=>{
  ...
  if(isAdmin(msg.from.id)&&adminStates[c]){
    ...
    if(st.action==="sch_text"&&msg.text){await setSetting(`scholarship_${st.lang}`,msg.text);if(st.fileId)await setSetting(`scholarship_file_${st.lang}`,st.fileId);adminStates[c]=null;return bot.sendMessage(c,"✅");}
