...
async function initDB() {
  ...
  try {
    await client.query(`
      ...
      CREATE TABLE IF NOT EXISTS legacy (id SERIAL PRIMARY KEY, type VARCHAR(20) NOT NULL, title TEXT NOT NULL, description TEXT DEFAULT '', year TEXT DEFAULT '', lang VARCHAR(5) DEFAULT 'uz', file_id TEXT DEFAULT '');
      ...
      CREATE TABLE IF NOT EXISTS quizzes (id SERIAL PRIMARY KEY, question TEXT NOT NULL, option_a TEXT NOT NULL, option_b TEXT NOT NULL, option_c TEXT NOT NULL, option_d TEXT NOT NULL, correct VARCHAR(1) NOT NULL, lang VARCHAR(5) DEFAULT 'uz');
...
async function getUserQuizResults(uid){const r=await q("SELECT COUNT(*) as t,SUM(CASE WHEN is_correct THEN 1 ELSE 0 END) as c FROM quiz_results WHERE user_id=$1",[uid]);return{total:parseInt(r.rows[0].t),correct:parseInt(r.rows[0].c||0)};}
...
bot.onText(/\/skip/,async(m)=>{const c=m.chat.id;if(!adminStates[c])return;
  ...
  if(adminStates[c].action==="sch_step1"){adminStates[c]={action:"sch_step1_lang",fileId:""};return bot.sendMessage(c,"Til tanlang:",{reply_markup:{inline_keyboard:[[{text:"🇺🇿",callback_data:"adm_sch_uz"},{text:"🇷🇺",callback_data:"adm_sch_ru"},{text:"🇬🇧",callback_data:"adm_sch_en"}]]}});}
